...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
    ...
    <xsl:sort select="@Name" order="ascending"/>
        ...
        <xsl:if test="not(@solution)">Projet : <xsl:value-of select="@name"/>
                ...
                <tr class="row">
                    ...
                    <td class="content">
                        <xsl:if test="count(event[@error-level='3'])=1">
                            ...
                            <xsl:if test="@description='Converted'">Converti</xsl:if>
                            <xsl:if test="@description!='Converted'"><xsl:value-of select="@description"/></xsl:if>
                            </xsl:for-each>
                        </xsl:if>
                        <xsl:if test="count(event[@error-level='3'])!=1 and count(event[@error-level='3' and @description='Converted'])!=0">Converti
                        </xsl:if>
                    </td>
                ...
                <tr class="collapsed" bgcolor="#ffffff">
                    ...
                    <td colspan="7">
                        <table width="97%" border="1" bordercolor="#dcdcdc" rules="cols" class="issuetable">
                            <tr>
                                <td colspan="7" class="issuetitle" _locID="ConversionIssues">Problèmes de conversion - <xsl:value-of select="@name"/>:</td>
                            </tr>
            ...
            <tr valign="top">
                <td class="foot">
                    <xsl:if test="count(source)!=1">
                        <xsl:value-of select="count(source)"/> fichiers
                    </xsl:if>
                    <xsl:if test="count(source)=1">
                        1 fichier
                    </xsl:if>
                ...
                <td class="foot">
                    Converti : <xsl:value-of select="count(source/event[@error-level='3' and @description='Converted'])"/><BR/>
                    Non converti <xsl:value-of select="count(source) - count(source/event[@error-level='3' and @description='Converted'])"/>
                </td>
    ...
    <xsl:template match="UpgradeLog">
        <html>
            ...
            <body topmargin="0" leftmargin="0" rightmargin="0" onclick="outliner();">
                ...
                <p><span class="note">
                <b>Heure de la conversion :</b>  <xsl:value-of select="Properties/Property[@Name='Date']/@Value"/>  <xsl:value-of select="Properties/Property[@Name='Time']/@Value"/><br/>
                </span></p>
