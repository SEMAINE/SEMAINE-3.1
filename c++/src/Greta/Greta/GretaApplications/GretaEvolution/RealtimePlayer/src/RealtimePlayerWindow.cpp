...
RealtimePlayerWindow::RealtimePlayerWindow():Fl_Double_Window(100,100,640,480,"Greta Realtime Player")
{
...
//	mutex=(SDL_mutex *)SDL_CreateMutex();
	...
	psydule=new Psydule(GretaName+REALTIMEPLAYERPSYDULE_MODULE_NAME,host,port);

	std::list<JString> datatypes;

	datatypes.push_back(GretaName+PSYCLONE_CLOCK_MESSAGE_HEADER);
	datatypes.push_back(GretaName+PSYCLONE_FAP_FRAME);
	datatypes.push_back(GretaName+PSYCLONE_BAP_FRAME);
	datatypes.push_back(GretaName+PSYCLONE_REALTIME_PLAYER_COMMAND);
	datatypes.push_back(GretaName+PSYCLONE_DATA_WAV);

	if(psydule->Register(GretaName+PSYCLONE_GRETA_WHITEBOARD,datatypes)!=0)
	{
		output->insert("registered to Psyclone");
		//SDL_CreateThread(realtimeplayerreceiver,this);
	}
	else
	{
		output->insert("could not register to Psyclone");
		psydule=0;
	}
	...
	fm=new FramesManager(&(this->pc));
	fm->start();

	cm=new CommandsManager(&(this->pc));

	receiver=new Receiver(psydule,fm,cm,&(pc),GretaName);
	receiver->start();
