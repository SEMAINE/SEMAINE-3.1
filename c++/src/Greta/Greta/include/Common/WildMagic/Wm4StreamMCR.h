...
// change was required to support 64-bit platforms, where pointers are 8
// bytes in size.
//
// The stream version (for .wmof files) before this change was 4.02 but was
// incremented to 4.03.  After the change, the engine can no longer read the
// version 4.02 files.  However, you can automatically convert them as
// follows.
//
// First, expose the #define of WM4_HACK_TO_CONVERT_TO_403 by removing the
// comment symbols before it.  Rebuild LibGraphics, the renderer libraries,
// and the application libraries.
//
// Second, modify and run the console application
//     GeometricTools/WildMagic4/Data/Wmof/ConvertToVersion403
// This application has a list of version 4.02 .wmof files that need to be
// converted.  You must copy your .wmof files to the Wmof directory, modify
// the list of filenames, and run the application.

//#define WM4_HACK_TO_CONVERT_TO_403
#ifdef WM4_HACK_TO_CONVERT_TO_403
extern int GetHackPtrSizeOf(size_t);
#define WM4_PTRSIZE(value) GetHackPtrSizeOf(sizeof(value))
#else
