...
int cPitchSmoother::setupNewNames(long nEl)
{
  ...
  if (F0final) { writer->addField("F0final", 1); n++; } 
  if (F0finalEnv) { writer->addField("F0finEnv", 1); n++; } 
  if (voicingFinalClipped) { writer->addField("voicingFinalClipped", 1); n++; } 
  if (voicingFinalUnclipped) { writer->addField("voicingFinalUnclipped", 1); n++; } 
  ...
  long i,idx,nC;
  nInputLevels = reader->getNLevels();
  ...
  for (i=0; i<nInputLevels; i++) {
    ...
    cVectorMeta *mdata = reader->getLevelMetaDataPtr(i);
    if (mdata != NULL) {
      // TODO : check mdata ID
