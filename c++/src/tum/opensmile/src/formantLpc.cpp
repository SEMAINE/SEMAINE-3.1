...
int cFormantLpc::setupNewNames(long nEl)
{
  ...
  if (saveIntensity) {
    writer->addField("formantFrameIntensity", 1); n+=1;
  }

  if (saveNumberOfValidFormants) {
    writer->addField( "nFormants", 1 ); n += 1;
  }
  if (saveFormants) {
    writer->addField( "formantFreqLpc", nFormants ); n += nFormants;
  }
  if (saveBandwidths) {
    writer->addField( "formantBandwidthLpc", nFormants ); n += nFormants;
  }
...
int cFormantLpc::processVectorFloat(const FLOAT_DMEM *src, FLOAT_DMEM *dst, long Nsrc, long Ndst, int idxi) // idxi=input field index
{
  ...
  if (useLpSpec) { /* formant computation by peak picking in lp spectrum and interpolation */
    ...
    gsl_poly_complex_workspace * w = gsl_poly_complex_workspace_alloc(nLpc+1);
    gsl_poly_complex_solve (lpc, nLpc+1, w, roots);
