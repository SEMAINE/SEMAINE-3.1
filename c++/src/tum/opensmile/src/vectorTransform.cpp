...
int cVectorTransform::loadMVNdata(const char *filename, struct sTfData * tf)
{
  ...
  if (f!=NULL) {
    ...
    if (filename != NULL) {
      SMILE_ERR(1,"cannot open transform data initialisation file '%s' for reading it as MVN binary format",filename);
      ret = 0;
    } else {
      ret = 1;
    }
...
int cVectorTransform::loadSMILEtfData(const char *filename, struct sTfData * tf)
{
  ...
  if (f!=NULL) {
    ...
    if (filename != NULL) {
      SMILE_ERR(1,"cannot open transform data initialisation file '%s' for reading it as SMILEtf binary format",filename);
      ret = 0;
    } else {
      ret = 1;
    }
...
int cVectorTransform::loadTransformData(const char *filename, struct sTfData * tf)
{
    ...
    if ( (magic[0] == smileMagic[0]) &&
      ...
      if ( (magic[0] == '<') &&
         ...
         ( (magic[1] == 'A')||(magic[1] == 'a') ) ) {
          ...
          fclose(f);
          SMILE_IMSG(4,"loading init file in HTK CMN Ascii format");
          ret = loadHTKCMNdata(filename, tf);
      } else {
          /* load old format MVN binary file */
          fclose(f);
          SMILE_IMSG(4,"loading init file in old MVN binary format");
  ...
  SMILE_IWRN(2,"could not open initial transform file (initFile) '%s' for reading, probably 'file not found' !",filename);
  return 0;
}
