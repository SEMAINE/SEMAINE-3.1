...
#include <TumAudioFeatureExtractor.h>

#undef MODULE
#define MODULE "TumAudioFeatureExtractor.SEMAINEcomponent"
...
namespace semaine {
  namespace components {
    ...
    TumAudioFeatureExtractor::TumAudioFeatureExtractor(cComponentManager *_cMan, cConfigManager *_conf) throw(CMSException) : 
      ComponentForSmile("TumAudioFeatureExtractor",_cMan,_conf,true,false),
    ...
    TumAudioFeatureExtractor::~TumAudioFeatureExtractor()
    {
      if (cMan != NULL) {
        cMan->requestAbort();
    ...
    void TumAudioFeatureExtractor::customStartIO() throw(CMSException)
    {
      if (cMan == NULL) { SMILE_ERR(1,"componentManager (cMan) is NULL, smileMainThread can not be started!"); }
        ...
        /*#ifdef __WINDOWS
          SetThreadPriority( smileMainThread, 16 );
        #endif*/
    ...
    void TumAudioFeatureExtractor::react(SEMAINEMessage * m) throw (std::exception) 
    {
      ...
      */
      ...
      printf("Received a SEMAINE callback message from the player.\n"); fflush(stdout);
      ...
      if (xm == NULL) {
        ...
        throw semaine::cms::exceptions::MessageFormatException("expected XML message, got "+std::string(typeid(*m).name()));
      }
        ...
        XMLTool::getLocalName(callback) != "callback") {
          ...
          if (XMLTool::getNamespaceURI(callback) == SemaineML::namespaceURI ||
              ...
              if (user != NULL) {
                ...
                printf("SEMAINE callback , userPresence msg, msg type='%s'\n",ctype);
            ...
            throw semaine::cms::exceptions::MessageFormatException("Expected callback message, but found "+ XMLTool::getLocalName(callback) + " element in namespace " + XMLTool::getNamespaceURI(callback));
          }
    ...
    void TumAudioFeatureExtractor::act() throw(CMSException)
    {
