...
#ifndef SEMAINE_COMPONENTS_SMILE_TUMOPENSMILE_H
#define SEMAINE_COMPONENTS_SMILE_TUMOPENSMILE_H
...
#include <semaineEmotionSender.hpp>
#include <semaineUserinfoSender.hpp> // VAD, gender, etc.
*/
...
#undef class
class TumAudioFeatureExtractor : public ComponentForSmile
{
public:
	TumAudioFeatureExtractor(cComponentManager *_cMan, cConfigManager *_conf) throw (CMSException);
 
	virtual ~TumAudioFeatureExtractor();
...
private:
	FeatureSender * featureSender; // low-level active mq feature sender

	XMLReceiver * callbackReceiver; // receiver for wave output callback messages
