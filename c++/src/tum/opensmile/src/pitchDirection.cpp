...
int cPitchDirection::setupNewNames(long nEl) 
{
  ...
  if (F0directionOutp) {
    writer->addField("F0direction"); n++;
  }
  if (directionScoreOutp) { // smean-lmean
    writer->addField("directionScore"); n++;
  }
  if (speakingRateOutp) {
    writer->addField("speakingRate"); n++;
  }
  if (F0avgOutp) {
    writer->addField("F0avg"); n++;
  }
  if (F0smoothOutp) {
    writer->addField("F0smooth"); n++;
  }
...
int cPitchDirection::myTick(long long t)
{
  ...
  */
  ...
  if (nBuf0 < speakingRateBsize) {
    nBuf0++; 
    if (nBuf0 == speakingRateBsize) {
      curSpkRate = (double)nSyl0 / ( (double)nBuf0 * reader->getLevelT());
    }
  }
  if (nBuf1 < speakingRateBsize*2) nBuf1++;
  else {
    curSpkRate = (double)nSyl1 / ( (double)nBuf1 * reader->getLevelT());
    nBuf1 -= nBuf0;
    nSyl1 -= nSyl0;
    nSyl0 = 0;
    nBuf0 = 0;
