...
int cFormantSmoother::setupNewNames(long nEl)
{
  ...
  if (intensity) { writer->addField("formantFrameIntensity", 1); n++; } 
  if (nFormants > 0) {
    if (saveEnvs) {
      if (formants) { writer->addField("formantFinFreqEnv", nFormants); n += nFormants; } 
      if (bandwidths) { writer->addField("formantFinBWEnv", nFormants); n += nFormants; } 
    } else {
      if (formants) { writer->addField("formantFinalFreq", nFormants); n += nFormants; } 
      if (bandwidths) { writer->addField("formantFinalBW", nFormants); n += nFormants; } 
    }
  }
