...
int ConfigType::setField(const char *_name, const char *description, int type, int subtype, const ConfigType *subType, int freeType, int _N, int printDflt)
{
...
int ConfigType::findField(const char *_name, int *arrI, char **arrS) const
{
  ...
  if (name == NULL) {
    SMILE_DBG(7,"ConfigType::findField: called with _name == NULL!");
    return -1;
  }
...
int cFileConfigReader::openInput(const char*fname, int *idx0)
{
  ...
  if (fname==NULL) {
    ...
    if (in == NULL) {
      ...
      if (fname3 != NULL) {
        SMILE_MSG(2,"config file '%s' not found, trying to open '%s'.",fname,fname3);
        in = fopen( fname3, "r" );
        if (in == NULL) {
