...
public:
  ...
  void setPeepWeights(FLOAT_NN *x, long N, int copy=1);
  ...
  ~cNnRnn() {
    ...
    if (connection != NULL) {
      for (i=0; i<nConnections; i++) {
        if (connection[i] != NULL) delete connection[i];
      }
      free(connection);
    }
  }
...
public:
  ...
  char from[102];
  char to[102];
  unsigned long F,T; // From, To as int constants (or'ed)
