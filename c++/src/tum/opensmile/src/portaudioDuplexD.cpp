...
portaudio dataPorcessor for semi-duplex recording and playback from the same audio device using two portaudio streams
...
#define MODULE "cPortaudioDuplexD"

#ifdef HAVE_PORTAUDIO

SMILECOMPONENT_STATICS(cPortaudioDuplexD)

SMILECOMPONENT_REGCOMP(cPortaudioDuplexD)
{
  SMILECOMPONENT_REGCOMP_INIT

  scname = COMPONENT_NAME_CPORTAUDIODUPLEXD;
  sdescription = COMPONENT_DESCRIPTION_CPORTAUDIODUPLEXD;
...
//    ct->setField("buffersize","(initial) (ring-)buffer size of wave writer in samples (default determined by 3*audioBuffersize)",0);
  ...
  SMILECOMPONENT_MAKEINFO(cPortaudioDuplexD);
}

SMILECOMPONENT_CREATE(cPortaudioDuplexD)

//-----

cPortaudioDuplexD::cPortaudioDuplexD(const char *_name) :
...
nBits(16),
isPaInit(0),
droppedW(0), droppedR(0),
callbackMatrix(NULL),
lastDataCount(0),
streamIn(NULL), streamOut(NULL),
streamStatusIn(PA_STREAM_STOPPED), streamStatusOut(PA_STREAM_STOPPED)
{
  smileMutexCreate(callbackMtx);
  smileCondCreate(callbackCond);
}

void cPortaudioDuplexD::fetchConfig()
{
...
int cPortaudioDuplexD::configureWriter(sDmLevelConfig &c)
{
...
int cPortaudioDuplexD::myConfigureInstance()
{
  int ret = 1;
  ...
  if (listDevices) {
    if (listDevices != -1) {
      printDeviceList();
      listDevices=-1;
      SMILE_IMSG(1,"Device list was printed, cPortaudioDuplexD::myConfigureInstance will now return 0 in order to stop openSMILE initialisation!");
    }
    return 0;
  } else {
    if (numDevices <= 0) {
...
int cPortaudioDuplexD::setupNewNames(long nEl)
{
...
int cPortaudioDuplexD::myFinaliseInstance()
{
...
int cPortaudioDuplexD::myTick(long long t)
{
  if (isEOI()) return 0; //XXX ????

  if (streamStatusOut == PA_STREAM_STOPPED) {
    dataFlagW = 1;
    if (!startPlayback()) return 0;
  }
  if (streamStatusIn == PA_STREAM_STOPPED) {
    dataFlagR = 1;
...
cPortaudioDuplexD::~cPortaudioDuplexD()
{
  if (isPaInit) {
    smileMutexLock(callbackMtx);
    stopRecording();
...
int matrixToPcmDataFloat_dd(void *outputBuffer, long __N, cMatrix *_mat, int channels, int mixdown=0) 
{
  int i,c,n;
...
// channels = number of output channels (can be 1 or 2)
int pcmDataFloatToMatrix_dd(const void *inputBuffer, long __N, cMatrix *_mat, int channels, int mixdown=0) 
{
  int i,c,n;
...
*/
static int paDuplexD_recordCallback( const void *inputBuffer, void *outputBuffer,
                           ...
                           void *userData )
{
  cPortaudioDuplexD * obj = (cPortaudioDuplexD *)userData;
  ...
  if (writer->checkWrite(framesPerBuffer)) {
    ...
    pcmDataFloatToMatrix_dd(inputBuffer, framesPerBuffer, mat,  obj->getChannels(), obj->isMonoMixdownREC());
    ...
    writer->setNextMatrix(mat);
	  ...
	  //if (obj->dataFlagR == 1) 
	    SMILE_MSG(3,"dropped (>=%i) frames during recording! (maybe increase audioBuffersize ?) [instance: '%s']",framesPerBuffer,obj->getInstName());
    obj->droppedR += framesPerBuffer;
    obj->dataFlagR = 0;
  }
...
** that could mess up the system like calling malloc() or free().
*/
static int paDuplexD_playbackCallback( const void *inputBuffer, void *outputBuffer,
                           unsigned long framesPerBuffer,
#ifdef HAVE_PORTAUDIO_V19
                           const PaStreamCallbackTimeInfo* timeInfo,
                           PaStreamCallbackFlags statusFlags,
                           ...
                           void *userData )
{
  ...
  if (_mat != NULL) {
    matrixToPcmDataFloat_dd(outputBuffer, framesPerBuffer, _mat, obj->getChannels(), obj->isMonoMixdownPB());
	  obj->dataFlagW=1;
    smileCondSignal(obj->callbackCond);
  } else {
    // we missed data....
...
int cPortaudioDuplexD::startPlayback()
{
  PaStreamParameters  outputParameters;
  PaError             err = paNoError;

#ifdef HAVE_PORTAUDIO_V19
  if (Pa_IsStreamActive( streamOut ) == 1) {
#else
  if (Pa_StreamActive( streamOut ) == 1) {
#endif
  ...
  streamStatusOut = PA_STREAM_STARTED;
  smileMutexUnlock(callbackMtx);

  return 1;
}

int cPortaudioDuplexD::startRecording()
{
  PaStreamParameters  inputParameters;
  PaError             err = paNoError;

#ifdef HAVE_PORTAUDIO_V19
  if (Pa_IsStreamActive( streamIn ) == 1) {
#else
  if (Pa_StreamActive( streamIn ) == 1) {
#endif
  ...
  if( err != paNoError ) COMP_ERR("cannot start portaudio stream (code %i)\n",err);
  streamStatusIn = PA_STREAM_STARTED;
  smileMutexUnlock(callbackMtx);

  return 1;
}

int cPortaudioDuplexD::stopRecording()
{
  PaError             err = paNoError;
  if (streamStatusIn == PA_STREAM_STOPPED) return 0;

  streamStatusIn =  PA_STREAM_STOPPED;
  abort = 1;

  err = Pa_CloseStream( streamIn );
  if( err != paNoError ) {
    SMILE_IERR(1,"cannot close portaudio input stream (code %i)\n",err);
    return 0;
  }
...
int cPortaudioDuplexD::stopPlayback()
{
  PaError             err = paNoError;
  if (streamStatusOut == PA_STREAM_STOPPED) return 0;

  streamStatusOut =  PA_STREAM_STOPPED;
  abort = 1;

  err = Pa_CloseStream( streamOut );
  if( err != paNoError ) {
    SMILE_IERR(1,"cannot close portaudio output stream (code %i)\n",err);
    return 0;
  }
...
int cPortaudioDuplexD::stopRecordingWait()
{
  PaError             err = paNoError;
  if (streamStatusIn == PA_STREAM_STOPPED) return 0;
...
int cPortaudioDuplexD::stopPlaybackWait()
{
  PaError             err = paNoError;
  if (streamStatusOut == PA_STREAM_STOPPED) return 0;
...
void cPortaudioDuplexD::printDeviceList(void)
{
  // query devices:
