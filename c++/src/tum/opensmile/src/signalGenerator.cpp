...
void cSignalGenerator::fetchConfig()
{
  ...
  if (isSet("lengthFrames")) {
    ...
    if (_length < 0.0) lengthFrames = -1;
      ...
      if (period > 0.0) {
        lengthFrames = (long)ceil( _length / period );
      } else {
...
//  samplePeriod = writer->getLevelT();
  ...
  for (i=0; i<nFields; i++) {
    writer->addField(fieldNames[i],nElements[i]);
    nValues += nElements[i];
  }
...
int cSignalGenerator::myTick(long long t)
{
  ...
  if ( (nValues > 0) && ( (curT < lengthFrames) || (lengthFrames == -1) ) && (writer->checkWrite(1)) ) {
    long i=0;
    const static int q = 15;
    const static float c1 = (1 << q) - 1;
