...
cMatrix * cDataReader::getMatrix(long vIdx, long _length, int special, int privateVec) // vIdx: start index of matrix (absolute)
{
  ...
  if (nLevels > 1) {
    ...
    for (i=0; i<nLevels; i++) {
      r &= dm->checkRead(level[i],vIdx,special,rdId[i],_length);
    }
    if (r) {
      if (_m!=NULL) {
        if (_length != _m->nT) {
      ...
      if (_m == NULL) _m = new cMatrix(myLcfg->N,_length,myLcfg->type);
      ...
      FLOAT_DMEM*df = _m->dataF;
      INT_DMEM*di = _m->dataI;
      long N = myLcfg->N;
      long f=0;
      for (i=0; i<nLevels; i++) {

        cMatrix *m2 = dm->getMatrix(level[i],vIdx,vIdx+_length, special, rdId[i]);
        if (m2 != NULL) {
          // copy data from f2 into V at the correct position
        ...
        if (i==0) _m->tmetaClone(m2->tmeta); // TODO: change this for asynchronous levels...
    ...
    cMatrix *m2 = dm->getMatrix(level[0],vIdx,vIdx+_length, special, rdId[0]);
...
cMatrix * cDataReader::getNextMatrix(int privateVec)
{
  //printf("here: 0.0.0\n");
  if (stepM == 0) { // read complete input...
    //printf("here: 0.0\n");
    if (isEOI()) {
      //printf("here: 0\n");
      int i;
    ...
    cMatrix *ret = getMatrix(curR,lengthM,-1,privateVec);
