...
j_open_stream(Recog *recog, char *file_or_dev_name)
{
  ...
  if (jconf->input.type == INPUT_WAVEFORM) {
    ...
    case SP_EXTERN:
      /* read parameter file */
      param_init_content(recog->mfcclist->param);
      recog->mfcclist->ext.fv_read = recog->jconf->ext.fv_read;
      recog->mfcclist->ext.userptr = recog->jconf->ext.userptr;
//????
    //  if (rdparam(file_or_dev_name, recog->mfcclist->param) == FALSE) {
//	jlog("ERROR: error in reading parameter file: %s\n", file_or_dev_name);
//	return -1;
...
#ifdef EXTERNAL_FV
  if (jconf->input.type == INPUT_WAVEFORM || jconf->input.speech_input == SP_MFCMODULE || jconf->input.speech_input == SP_EXTERN) {
#else
