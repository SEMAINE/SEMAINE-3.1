 ...
 *
 * @@ This file was modified by Florian Eyben from TUM Germany, July 2009,
 *    to add a mechanism to pass features from external code to julius @@
 *
 * $Revision: 1.5 $
 * 
 */
...
RealTimePipeLinePrepare(Recog *recog)
{
  ...
  for(mfcc = recog->mfcclist; mfcc; mfcc = mfcc->next) {
    ...
    if (0) { } else  // only to make compiler happy in all possible configurations!
#ifdef ENABLE_PLUGIN
...
#endif
#ifdef EXTERNAL_FV
    if (recog->jconf->input.speech_input == SP_EXTERN) {

      /* vector length in unit */
      mfcc->param->veclen = recog->jconf->ext.veclen;
...
RealTimeCMNUpdate(MFCCCalc *mfcc, Recog *recog)
{
 ...
 // if(para->cmn) {
  // TODO: don't use para when SP_EXTERN... configure via mfcc or recog pointer
  if (1) {
    if (mfcc->cmn.update) {
      cmn_update_p = TRUE;
...
mfcc_go(Recog *recog, int (*ad_check)(Recog *))
{
  ...
  while(1/*in_data_vec*/) {
	...
	if ((ret3 == -1 && recog->mfcclist->f == 0) || ret3 == -2) { /* <<- !! recog->mfcclist->f changed from 'mfcc->f' by Flo */
	  return(-2);
	}
      }
    }
...
#ifdef ENABLE_PLUGIN
      /* call post-process plugin if exist */
      plugin_exec_vector_postprocess(recog->mfcclist->param->parvec[recog->mfcclist->f], recog->mfcclist->param->veclen, recog->mfcclist->f); /* <<- !! last parameter changed from 'mfcc->f' by Flo */
#endif
